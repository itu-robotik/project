...
class AutoExplorer(Node):
    def __init__(self):
        ...
        self.start_delay_counter = 5 # Wait 5 seconds before starting
    ...
    def timer_callback(self):
        ...
        if not self.odom_received:
            self.get_logger().warn('Waiting for Odometry (Robot Position)...', throttle_duration_sec=2.0)
            return

        if self.start_delay_counter > 0:
            self.get_logger().info(f'Starting in {self.start_delay_counter} seconds...', throttle_duration_sec=1.0)
